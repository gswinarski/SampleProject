...
## Where notes live
One file per object, under `objects/`, named `salesforce-<object>.md` (for example `salesforce-case.md`). The `salesforce-` prefix is deliberate: this repository is one Obsidian vault, and a bare name like `case.md` risks colliding with an unrelated document elsewhere in `docs/` — two files with the same basename would make `[[wikilinks]]` ambiguous. Non-Salesforce systems, if ever documented here, should get their own prefix (`oss-...`, `nms-...`) for the same reason.
...
- [[salesforce-incident]] — the Incident object, as used by [Planned works and mass outage notification](doc:TOPIC-OUTAGE)
- [[salesforce-asset]] — the Asset object representing a customer's service, and its network identifier
