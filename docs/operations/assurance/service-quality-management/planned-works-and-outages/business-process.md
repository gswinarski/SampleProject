...
## Rules
An Incident is created from network conditions, not from a customer Case. A Case may separately reference an open Incident; that referencing is out of scope for this process, which only creates, updates and closes the Incident. The portal never shows raw network diagnostic detail, only a customer-safe summary, the affected window and the current status. A repeated or replayed NMS event never creates a second Incident for the same condition.
